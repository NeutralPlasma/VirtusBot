...
class PMRemoveXpCommand (private var playerManager: PlayerLeveling): SubCommand(){
    ...
    override fun execute(event: SubCommandEvent) {
        ...
        if(users.isEmpty()){
            ...
            MessageHandler.sendSuccess("Successfully removed xp from user ${users[0].effectiveName}", mevent.textChannel)
